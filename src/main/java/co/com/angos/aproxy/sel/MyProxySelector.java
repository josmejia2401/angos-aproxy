package co.com.angos.aproxy.sel;
...
public class MyProxySelector extends ProxySelector {
	...
	/*
	 * Method called by the handlers when it failed to connect to one of the proxies
	 * returned by select().
	 */
	public void connectFailed(URI uri, SocketAddress sa, IOException ioe) {
		...
		if (p != null) {
			/*
			 * It's one of ours, if it failed more than 3 times let's remove it from the
			 * list.
			 */
			if (p.failed() >= 3)
				proxies.remove(sa);
